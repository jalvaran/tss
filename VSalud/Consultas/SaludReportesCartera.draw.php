...
if( !empty($_REQUEST["TipoReporte"]) ){
    ...
    switch ($_REQUEST["TipoReporte"]) {
        case 1: //Pagos totales
            ...
            $query = "SELECT COUNT(*) as `num`,SUM(TotalPagos) AS TotalPagado,SUM(DiferenciaPagos) AS DiferenciaPagos FROM {$statement}";
            $row = $obGlosas->FetchArray($obGlosas->Query($query));
            $ResultadosTotales = $row['num'];
            $TotalPagado=$row['TotalPagado'];
            $DiferenciaPagos=$row['DiferenciaPagos'];
            ...
            if($obGlosas->NumRows($consulta)){
                ...
                $css->FilaTabla(14);
                    ...
                    print("</td>");
                    print("<td style='text-align:center'>");
                        print("<strong>Diferencia en Pagos:</strong> <h4 style=color:green>". number_format($DiferenciaPagos)."</h4>");
                    print("</td>");
                    print("<td colspan='5'>");
                ...
                if($Resultados){
                    ...
                    $css->FilaTabla(14);
                        ...
                        $css->ColTabla("<strong>DIFERENCIA EN PAGOS</strong>", 1);
                    ...
                    while($DatosConsulta=$obGlosas->FetchAssoc($consulta)){
                        $css->FilaTabla(12);
                            ...
                            $css->ColTabla(number_format($DatosConsulta["DiferenciaPagos"]), 1);
        ...
        case 2: // Facturas no pagas
            ...
            $query="SELECT NitEPS,CuentaRIPS,CuentaGlobal,razon_social,num_factura,fecha_factura,fecha_radicado,DiasMora ,tipo_negociacion,"
        ...
        case 3: // Facturas pagas con diferencias
            ...
            $VectorST = explode("LIMIT", $statement);
            $statement = $VectorST[0]; 
            $query = "SELECT COUNT(*) as `num`,SUM(totalPagos) AS TotalXPagar,SUM(DiferenciaEnPago) as Diferencia FROM {$statement}";
            $row = $obGlosas->FetchArray($obGlosas->Query($query));
            $ResultadosTotales = $row['num'];
            $TotalXPagar=$row['TotalXPagar'];
            ...
            if($obGlosas->NumRows($consulta)){
                ...
                $css->FilaTabla(14);
                    ...
                    print("</td>");
                    print("<td style='text-align:center'>");
                        print("<strong>Saldo a pagar:</strong> <h4 style=color:orange>". number_format($Diferencia)."</h4>");
                    print("</td>");
                    print("<td colspan='6'>");
        ...
        case 5: // Cartera X Edades
            ...
            $sql="CREATE VIEW vista_salud_carteraxdias_v2 AS 
                ...
                (SELECT DATEDIFF('$FechaCorte',t1.`fecha_radicado` ) - (SELECT dias_convenio FROM salud_eps t2 WHERE t2.cod_pagador_min=t1.cod_enti_administradora LIMIT 1)) as DiasMora ,t1.`CuentaRIPS` as CuentaRIPS,t1.`CuentaGlobal` as CuentaGlobal,t1.`cod_prest_servicio`,t1.`razon_social`,t1.`num_factura`,
                ...
                (SELECT nombre_completo FROM salud_eps WHERE salud_eps.cod_pagador_min=t1.cod_enti_administradora LIMIT 1) as RazonSocialEPS,
                (SELECT nit FROM salud_eps WHERE salud_eps.cod_pagador_min=t1.`cod_enti_administradora`) as NIT_EPS,
                (SELECT tipo_regimen FROM salud_eps WHERE salud_eps.cod_pagador_min=t1.`cod_enti_administradora`) as Regimen,
        ...
        while($DatosEPS=$obGlosas->FetchArray($ConsultaCartera)){
            ...
            $css->FilaTabla(14);
                ...
                $Datos["TotalFacturas"]=$CatidadFacturas;
        ...
        case 9: // informe de margen de facturacion de capita
            ...
            $statement=" `vista_porcentaje_capitas_reporte` $Condicional $Condicional2";
            ...
            $VectorST = explode("LIMIT", $statement);
            $statement = $VectorST[0]; 
            $query = "SELECT COUNT(*) as `num`,SUM(DiferenciaFacturacion) AS Diferencia FROM {$statement}";
            $row = $obGlosas->FetchArray($obGlosas->Query($query));
            $ResultadosTotales = $row['num'];
            $TotalXPagar=$row['Diferencia'];
            $st_reporte=$statement;
            $Limit=" ORDER BY DiferenciaFacturacion ASC LIMIT $startpoint,$limit";
            ...
            if($obGlosas->NumRows($consulta)){
                ...
                $css->FilaTabla(12);
                    ...
                    print("</td>");
                    print("<td colspan='11'>");
                        $css->CrearNotificacionRoja("Porcentaje Capita Facturada", 16);
                    print("</td>");
                    print("<td colspan='3' style='text-align:center'>");
                        $st1= urlencode($st_reporte);
                        $css->CrearImageLink("ProcesadoresJS/GeneradorCSVReportesCartera.php?Opcion=$TipoReporte&sp=$Separador&st=$st1", "../images/csv.png", "_blank", 50, 50);
                ...
                if($Resultados){
                    ...
                    $css->FilaTabla(12);
                        ...
                        $css->ColTabla("<strong>VALOR SERVICIOS PRESTADO</strong>", 1);
                        $css->ColTabla("<strong>DIFERENCIA</strong>", 1);
                    ...
                    while($DatosConsulta=$obGlosas->FetchAssoc($consulta)){
                        $css->FilaTabla(12);
                            ...
                            $css->ColTabla($DatosConsulta["CuentaContable"], 1);
                            $css->ColTabla(number_format($DatosConsulta["valor_neto_pagar"]), 1);
                            $css->ColTabla(number_format($DatosConsulta["ValorAFCapita"]), 1);
                            $css->ColTabla(number_format($DatosConsulta["DiferenciaFacturacion"]), 1);
                            $css->ColTabla(($DatosConsulta["PorcentajeFacturado"]), 1);

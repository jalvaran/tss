...
class Glosas extends conexion{
    ...
    public function CarteraSegunDias($idEPS,$Condicion,$Vector) {
        $sql="SELECT SUM(valor_neto_pagar - ValorGlosaAceptada - TotalPagos - TotalDevolucion) AS Total, COUNT(num_factura) AS NumFacturas FROM vista_salud_carteraxdias_v2 $Condicion";
        $Consulta=$this->Query($sql);
